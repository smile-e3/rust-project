...
impl Editor {
    ...
    pub fn default() -> Self {
        /*
         * 使用std标准库的env模块
         * env::args():创建Args结构体，Args结构体是进程参数的迭代器，将参数转化为字符串
         * fn collect(B)(self)->B:将迭代器转化为集合
         */
        let args: Vec<String> = env::args().collect();
        let mut initial_status = String::from("HELP: Ctrl-S = save | Ctrl-Q = quit");
        let document = if args.len() > 1 {
            let file_name = &args[1];
            //Document::open(&file_name).unwrap_or_default() //unwrap_or_default:产生错误，将会返回一个默认document，错误将被丢弃
